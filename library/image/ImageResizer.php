...
class ImageResizer
{
	...
	public function resize($format, $name, $type = "jpg", $file = "flashfile", $folder="images/uploaded/")
	{
		...
		if($format->format != ResizeFormat::ORIGINAL)
		...
		if($format->format == ResizeFormat::INSCRIBE)
			$this->inscribe($format);
		if($format->format == ResizeFormat::DESCRIBE)
			$this->describe($format);
		if($format->format == ResizeFormat::DESCRIBE_W)
			$this->describeW($format);
		if($format->format == ResizeFormat::DESCRIBE_H)
